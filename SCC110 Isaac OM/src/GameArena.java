...
public class GameArena extends JPanel implements Runnable, KeyListener, MouseListener, MouseMotionListener
{
	...
	private Image backgroundImage = null;
      //new
	  private boolean up1_Var, down_1_Var, left_1_Var, right_1_Var, up_2_Var, down_2_Var, left_2_Var, right_2_Var;
	...
	public int[] p_Inputs(){
		int x1_Var,y1_Var,x2_Var,y2_Var;
		// Calculate the vertical input for player 1
		y1_Var = (up1_Var ? -1 : 0) + (down_1_Var ? 1 : 0);

		// Calculate the horizontal input for player 1
		x1_Var = (left_1_Var ? -1 : 0) + (right_1_Var ? 1 : 0);

		// Calculate the vertical input for player 2
		y2_Var = (up_2_Var ? -1 : 0) + (down_2_Var ? 1 : 0);

		// Calculate the horizontal input for player 2
		x2_Var = (left_2_Var ? -1 : 0) + (right_2_Var ? 1 : 0);

		// Return the inputs as an integer array
		return new int[]{x1_Var, y1_Var, x2_Var, y2_Var};
	}
	/**
